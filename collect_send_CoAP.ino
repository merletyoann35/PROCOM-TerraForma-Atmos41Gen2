...
#define LORAWAN_APP_TX_DUTYCYCLE   300000   // 30000 = 30 sec , 300000 = 5 min
#define JOINREQ_NBTRIALS 3

// GLOBAL OBJECTS & VARIABLES
RAK_SDI12 mySDI12(RX_PIN, TX_PIN, OE);
// Global Message ID counter 
uint16_t globalMsgId = 0;
...
// Encodes an integer into CBOR format (handles negatives and size)
// Returns the number of bytes written
uint8_t encodeCBORInt(uint8_t *buf, int16_t val) {
    uint8_t len = 0;
    ...
    if (val >= 0) {
        ...
        // CBOR stores -1 as 0, -2 as 1, etc. -> (-1 - val)
        int16_t posVal = -1 - val;
        
        if (posVal < 24) {
            buf[len++] = 0x20 | (uint8_t)posVal; // 0x20 is Major Type 1
        } else if (posVal <= 255) {
            buf[len++] = 0x38; // 0x20 | 0x18
            buf[len++] = (uint8_t)posVal;
        } else {
            buf[len++] = 0x39; // 0x20 | 0x19
            buf[len++] = (uint8_t)(posVal >> 8);
            buf[len++] = (uint8_t)(posVal & 0xFF);
        }
    }
    return len;
}
...
void loop() {
    ...
    if (startMeasurementCycle) {
        ...
        // Note: We do NOT delay here. We let the loop continue
        // to picking up characters in the next block.
    }
...
static void send_lora_frame(void) {
    ...
    uint8_t *buf = m_lora_app_data.buffer;
    uint8_t i = 0;
    
    // COAP HEADER (4 Bytes)
    // Byte 0: Ver(1) | Type(NON=1) | TKL(0) -> Binary: 01 01 0000 -> 0x50
    buf[i++] = 0x50; 
    // Byte 1: Code (POST = 0.02)
    buf[i++] = 0x02;
    // Byte 2-3: Message ID (Incrementing)
    globalMsgId++;
    buf[i++] = (uint8_t)(globalMsgId >> 8);
    buf[i++] = (uint8_t)(globalMsgId & 0xFF);

    // COAP OPTIONS
    // Option: Uri-Path (Number 11) = "weather"
    // Delta = 11, Length = 7. 
    // Since Delta < 13 and Len < 13, we pack them in one byte: (Delta<<4 | Len)
    buf[i++] = (11 << 4) | 7; 
    memcpy(&buf[i], "weather", 7);
    i += 7;

    // Option: Content-Format (Number 12) = 60 (CBOR)
    // Previous option was 11. Delta = 12 - 11 = 1.
    // Length of value '60' is 1 byte.
    // Byte: (Delta<<4 | Len) -> (1<<4 | 1) -> 0x11
    buf[i++] = 0x11;
    buf[i++] = 60; // Value: application/cbor

    // LAYER C: PAYLOAD MARKER
    buf[i++] = 0xFF;

    // LAYER D: CBOR PAYLOAD
    // Map of 6 items: 0xA6
    buf[i++] = 0xA6;

    // Rain (Key 1)
    buf[i++] = 0x01; 
    i += encodeCBORInt(&buf[i], (int16_t)(currentMeasure.rain_mm * 100));

    // Wind Dir (Key 2)
    buf[i++] = 0x02; 
    i += encodeCBORInt(&buf[i], (int16_t)currentMeasure.wind_dir_deg);

    // Wind Speed (Key 3)
    buf[i++] = 0x03; 
    i += encodeCBORInt(&buf[i], (int16_t)(currentMeasure.wind_speed_ms * 100));

    // Temp (Key 4)
    buf[i++] = 0x04;
    i += encodeCBORInt(&buf[i], (int16_t)(currentMeasure.temp_c * 100));

    // Humidity (Key 5)
    buf[i++] = 0x05;
    i += encodeCBORInt(&buf[i], (int16_t)(currentMeasure.humidity_pct * 100));

    // Pressure (Key 6)
    buf[i++] = 0x06;
    i += encodeCBORInt(&buf[i], (int16_t)(currentMeasure.pressure_kpa * 100));
    ...
    if (error == LMH_SUCCESS) {
        Serial.printf("[LORA] CoAP POST sent! Size: %d bytes\n", i);
    } else {
